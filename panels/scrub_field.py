...
class ScrubFieldController:
    ...
    def sync_all(self) -> bool:
        ...
        for state in self._fields.values():
            latest_spec = self._specs.get(state.prop)
            if latest_spec is not None and latest_spec != state.spec:
                state.spec = latest_spec
                changed = True
            if state.editing:
                continue
    ...
    def _on_body_mouseup(self, event) -> None:
        if self._active_prop is None:
            return
        open_editor = (
            self._fields.get(self._active_prop) is not None
            and not self._fields[self._active_prop].dragging
        )
